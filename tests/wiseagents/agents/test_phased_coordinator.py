...
def _check_messages(size: int, type: str, messages: list):
    _assert(len(messages) >= size, f"Unexpected {type} messages received by the Mock LLM {messages}")


def _check_agent_is_in_message(agent_name: str, agent_desc: str, message: str):
    _assert(f"Agent Name: {agent_name} Agent Description: {agent_desc}" in message)
...
def _mock_llm(*args, **kwargs):
    ...
    _assert(len(messages) > 1, f"Unexpected messages received by the Mock LLM {messages}")
    ...
    _check_messages(1, "system", system_messages)
    _assert(system_messages[0] == "You will be coordinating a group of agents to solve a problem.")
    _check_messages(1, "user", user_messages)
    _assert("determine all of the agents that could be required to solve the query" in user_messages[0])
    _assert("How do I prevent the following exception" in user_messages[0])
    _assert("NullPointerException" in user_messages[0])
    _assert("Available agents:" in user_messages[0])
    _check_agent_is_in_message("Coordinator", "This is a coordinator agent", user_messages[0])
    _check_agent_is_in_message("PassThroughClientAgent1", "This is a test agent", user_messages[0])
    _check_agent_is_in_message(
        "Agent2",
        "This agent provides information about error messages using Source1", user_messages[0])
    _check_agent_is_in_message(
        "Agent3",
        "This agent provides information about error messages using Source2", user_messages[0])
    _check_agent_is_in_message(
        "Agent4",
    ...
    _check_agent_is_in_message(
        ...
        "This agent should be called before calling Agent4.",
        user_messages[0])
    _check_messages(0, "assistant", assistant_messages)

    if cur_invocation == 1:
        _assert(len(messages) == 2, f"Unexpected messages received by the Mock "
    ...
    _check_messages(4, "all", messages)

    _check_messages(1, "system", system_messages)
    # System messages have not changed, no need to check again

    _check_messages(1, "assistant", assistant_messages)
    # The assistant message here will be what was returned by the previous step
    _assert(assistant_messages[0] == "Agent2 Agent3 Agent5 Agent4")

    _check_messages(2, "user", user_messages)
    # Check the new user message.
    _assert("Assign each of the agents that will be required to solve the query to" in user_messages[1])
    _assert("Information Gathering, Verification of Information, Analysis" in user_messages[1])

    if cur_invocation == 2:
        _assert(len(messages) == 4, f"Unexpected messages received by the Mock "
                                    f"LLM {messages} for invocation {cur_invocation}")
    ...
    _check_messages(7, "all", messages)

    _check_messages(3, "user", user_messages)
    # Check the new user message
    if not cur_invocation == 7:
        _assert(user_messages[2] ==
    ...
    _check_messages(2, "assistant", assistant_messages)
    # The new assistant message here will be what was returned by the previous step
    _assert(assistant_messages[1] == "Agent2 Agent3\nAgent5\nAgent4")
    ...
    if cur_invocation == 3 or cur_invocation == 4:
        ...
        _assert((cur_invocation == 3 and len(messages) == 7) or
                cur_invocation == 4 and len(messages) in (7, 8))

        _check_messages(2, "system", system_messages)
        ...
        if len(messages) == 8:
            _check_messages(3, "assistant", assistant_messages)
            assert assistant_messages[2] == "Agent2 Agent3\nAgent5\nAgent4"
    ...
    _check_messages(9, "all", messages)

    _check_messages(4, "assistant", assistant_messages)
    _assert(assistant_messages[1] == "Agent2 Agent3\nAgent5\nAgent4")
    latest_assistant_messages = assistant_messages[2:4]
    _assert("It is important to initialise all variables!" in latest_assistant_messages)
    _assert("Check if a variable is null before using it." in latest_assistant_messages)

    _check_messages(3, "user", user_messages)
    # user messages are the same as earlier
    if cur_invocation == 5:
        _assert(system_messages[1] ==
                "You will determine if the information provided by other agents is accurate.")
        _assert(len(messages) == 9)
        _check_messages(2, "system", system_messages)
        _assert(system_messages[1] == "You will determine if the information provided by other agents is accurate.")
    ...
    _check_messages(5, "assistant", assistant_messages)
    _assert(assistant_messages[4] == "It seems to avoid an NPE you should "
                                     "take care to initialise variables and check they are set before use.")

    if cur_invocation == 6:
        _assert(len(messages) == 10, messages)
        _check_messages(2, "system", system_messages)
        _assert(system_messages[1] == "You are a helpful assistant that will determine the underlying cause of "
    ...
    if cur_invocation == 7:
        _assert(len(messages) == 10, messages)
        _check_messages(5, "assistant", assistant_messages)
        _assert(assistant_messages[5] == "Look at line 47 of ExampleApp.java and check if the "
                                         "object is initialised before using it.")
        _assert(user_messages[2] ==
    ...
    _assert(False, f"Unexpected messages received by the Mock LLM {user_messages}")
